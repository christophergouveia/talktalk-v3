...
export default function Home({}) {
  ...
  const atualizarBandeira = (index: number) => {
  ...
  const handleAnimationUpdate = (progress: number, index: number) => {
    if (progress >= 0.99) {
      if (!completedAnimations.has(index)) {
        setCompletedAnimations((prev) => {
          const novo = new Set(prev);
          novo.add(index);
          return novo;
        });
        atualizarBandeira(index);
      }
          ...
          <div className="absolute inset-0 bg-gradient-to-br from-blue-50 to-cyan-50 dark:from-gray-900 dark:to-gray-800">
            <div className="absolute inset-0 animate-float opacity-30">
              {flags.map((flagData, i) => (
                <Tooltip key={i} delay={0} content={linguagens[i].label}>
                  <motion.div
                    key={i}
                    ...
                    onUpdate={(latest) => handleAnimationUpdate((latest.y as number) / -100, i)}
                  >
